...
function make_output_directories(data_save_directory)
...
function save_data(res, u_bests, u_names, config; file_name = "parameters.jld2")
    JLD2.@save file_name res u_bests u_names config
end

function load_data(file_name::String = "parameters.jld2")
    JLD2.@load file_name res u_bests u_names config
...
function ensemble_convergence_single_panel(
    ...
    anim_conv = Plots.@animate for i in 1:n_iter_p1
        u_i = u[i]
    ...
    for case_num in 1:n_cases
        ...
        for (i, _data) in enumerate([_fields_model, _fields_obs])
            ...
            for j in 1:n_variables_full
                ...
                _data_var = _data[j]
                if n_heights[j] == 1
                    _f_1d = Interpolations.LinearInterpolation(
                        _times,
                        _data_var[:],
                    ...
                    _f = Interpolations.LinearInterpolation(
                        (_heights, _times),
                        _data_var,
                        extrapolation_bc = Interpolations.Line(),
                    )
                ...
                if config["model"]["filter"]["apply"] == true
                    p[(i - 1) * n_variables + k] = Plots.heatmap(
                        _times_c ./ 60,
                        _heights ./ 1000,
                        _data_c,
                        xlabel = "time [min]",
...
function plot_box_results(
    ...
    for case_num in 1:n_cases
        ...
        for j in 1:n_variables
            Plots.plot(_times / 60, _fields_model[j][1, :], lw = 2, label = "model")
            Plots.plot!(
                _times / 60,
