...
function compute_loss(u::Array{Float64, 1}, u_names::Array{String, 1}, config::Dict, RS::ReferenceStatistics)
    ...
    loss = LA.dot(yg_diff, RS.Γ \ yg_diff) / n_cases
    return loss
end
...
function calibrate(::OptimStyle, priors, config, RS::ReferenceStatistics; verbose::Int = 1)
    ...
    _θ_inits = mapslices(x -> EKP.transform_constrained_to_unconstrained(priors, x), _u_inits; dims = 1)
...
function make_optim_loss_function(u_names::Array{String, 1}, priors, config::Dict, RS::ReferenceStatistics)
    function optim_loss_function(θ::Array{Float64, 1})
        u = mapslices(x -> EKP.transform_unconstrained_to_constrained(priors, x), θ; dims = 1)
        return compute_loss(u, u_names, config, RS)
    end
    return optim_loss_function
end

function get_results(optres::Optim.MultivariateOptimizationResults, priors::EKP.ParameterDistribution)
    _θ_optim = Optim.minimizer(optres)
    _ϕ_optim = mapslices(x -> EKP.transform_unconstrained_to_constrained(priors, x), _θ_optim; dims = 1)
    _cov_optim = Matrix(LA.I(length(_ϕ_optim))) .* eltype(_ϕ_optim)(1)
...
function calibrate(::EKPStyle, priors, config, ref_stats_list::Vector{ReferenceStatistics}; verbose::Int = 3)
    ...
    for n in 1:n_iter
        ...
        cases_indices_shuffled = Random.shuffle!(collect(1:n_cases))
        for j in 1:n_batches
            ...
            param_ensemble = [param_ensemble; [EKP.get_u_final(ekpobj)]]
        end
    end
...
function generate_ekp(
    ...
    if process_settings["augmented"]
        μ = vcat(Statistics.mean(priors)...)
        Γ_θ = Statistics.cov(priors)
        y = vcat(RS.y, μ)
    ...
    if process_settings["EKP_method"] == "EKI"
        ekpobj = EKP.EnsembleKalmanProcess(
            ...
            Γ,
            EKP.Inversion(),
            scheduler = EKP.DefaultScheduler(process_settings["Δt"]),
        )
    ...
    elseif process_settings["EKP_method"] == "UKI"
        process = EKP.Unscented(
            Statistics.mean(param_ensemble, dims = 2)[:],
            Statistics.cov(param_ensemble, dims = 2);
            α_reg = process_settings["α_reg"],
            update_freq = process_settings["update_freq"],
        )
        ekpobj = EKP.EnsembleKalmanProcess(y, Γ, process, scheduler = EKP.DefaultScheduler(process_settings["Δt"]))
    end
    return ekpobj
end

function get_ϕ_ensemble(param_ensemble::Matrix{FT}, priors::EKP.ParameterDistribution) where {FT <: Real}
    _ϕ_ensemble = zeros(size(param_ensemble))
    for j in 1:size(_ϕ_ensemble)[2]
        _ϕ_ensemble[:, j] = EKP.transform_unconstrained_to_constrained(priors, param_ensemble[:, j])
    end
    return _ϕ_ensemble
end

function get_ϕ_mean_final(param_ensemble::Vector{Matrix{FT}}, priors::EKP.ParameterDistribution) where {FT <: Real}
    _ϕ_mean = EKP.transform_unconstrained_to_constrained(priors, Statistics.mean(param_ensemble[end], dims = 2))[:]
    return _ϕ_mean
end

function get_results(param_ensemble::Vector{Matrix{FT}}, priors::EKP.ParameterDistribution) where {FT <: Real}
    _ϕ_optim = get_ϕ_mean_final(param_ensemble, priors)
    _cov_optim = Statistics.cov(param_ensemble[end], dims = 2)
    _cov_optim = _cov_optim ./ LA.tr(_cov_optim) .* length(_ϕ_optim)
    return (ϕ_optim = _ϕ_optim, cov_optim = _cov_optim)
end
