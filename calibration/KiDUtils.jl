...
function run_KiD(u::Array{FT, 1}, u_names::Array{String, 1}, model_settings::Dict)
    ...
    space, face_space =
        K1D.make_function_space(FT, model_settings["z_min"], model_settings["z_max"], model_settings["n_elem"])
    ...
    for i in 1:_nt_filtered
        ...
        for j in 1:_n_variables
            for k in 1:_nz_filtered[j]
                ...
                outputs = [outputs; Statistics.mean(_single_filtered_cell_data[_range, :])]
            end
        end
...
function create_kid_parameters(
    ...
    kid_params = K1D.Parameters.KinematicDriverParameters{FT}(;
        w1,
        t1,
        p0,
        z_0,
