...
function make_block_diagonal_matrix(a::AbstractMatrix, b::AbstractMatrix)
    ...
    if (a isa LA.Diagonal) && (b isa LA.Diagonal)
        return LA.Diagonal([LA.diag(a); LA.diag(b)])
    end
...
function compute_error_metrics(
    ...
    mse_m = LA.dot(yg_diff_mean_normalized, yg_diff_mean_normalized) / n_cases
    mse_s = LA.dot(yg_diff_std_normalized, yg_diff_std_normalized) / n_cases

    G_n = pca_transform(normalize_sim(G, RS), RS)
    yg_diff_n = G_n - RS.y
    loss = LA.dot(yg_diff_n, RS.Γ \ yg_diff_n) / n_cases
