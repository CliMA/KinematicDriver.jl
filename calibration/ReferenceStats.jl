...
Base.@kwdef struct ReferenceStatistics{FT <: Real}
    y::Vector{FT}
    Γ::AbstractMatrix{FT}
    P_pca::Union{AbstractMatrix{FT}, LA.UniformScaling}
    y_full::Vector{FT}
    ...
    function ReferenceStatistics(
        ...
        obs_mean = Statistics.mean(obs, dims = 2)[:]
        y_full = Statistics.mean(obs_normalized, dims = 2)[:]
        Γ_full = Statistics.cov(obs_normalized, dims = 2)
        ...
        if stats_config["tikhonov_mode"] == "relative"
            ...
            Γ = Γ + tikhonov_noise * eigmax(Γ) * LA.I
        else
            Γ = Γ + stats_config["tikhonov_noise"] * LA.I
        end

        LA.isposdef(Γ) ? nothing : println("WARNING: Covariance matrix Γ is ill-conditioned, consider regularization.")
...
function find_mean_and_std_maximum(
    ...
    for i in 1:n_cases
        ...
        _y_mean_vector_single_case = Statistics.mean(_y_single_case, dims = 2)
        ...
        _y_std_vector_single_case = Statistics.std(_y_single_case, dims = 2)
...
function normalize_obs(
    ...
    _y_normalized = centered ? y .- Statistics.mean(y, dims = 2) : copy(y)
    for i in 1:n_cases
