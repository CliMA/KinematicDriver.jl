...
function get_single_obs_field(
    ...
    for (i, var) in enumerate(variables)
        ...
        if length(size(_data)) == 1
            _f_1d = Interpolations.linear_interpolation(_t_data, _data, extrapolation_bc = Interpolations.Line())
            _f(t, z) = _f_1d(t)
        else
            _f =
                Interpolations.linear_interpolation((_t_data, _z_data), _data, extrapolation_bc = Interpolations.Line())
        end
        if apply_filter
...
function get_validation_samples(config)
    ...
    _Γy = convert(Array, LA.Diagonal((_G_t .* _G_t .+ eps(Float64))[:]))
    ...
    for i in 1:_n_samples
        _samples[:, i] = _G_t .+ rand(DS.MvNormal(_μ, _Γy)) .* scov_G_ratio
    end
...
function params_validation(config, priors = nothing)
    ...
    θ = priors === nothing ? 0 : EKP.ParameterDistributions.transform_constrained_to_unconstrained(priors, ϕ)
    return (ϕ = ϕ, θ = θ)
end
